/* EMBARCATECH - Transmissor LoRa para Estação Meteorológica
 * Envia temperatura (AHT20), pressão (BMP280), umidade (AHT20) e altitude via LoRa
...
int main() {
    stdio_init_all();
    sleep_ms(500);
    printf("Iniciando Transmissor LoRa (Pico W)\n");
    ...
    while (true) {
        ...
        double altitude = calculate_altitude(pressure);  // Corrigido: sem /1000.0, pressão em Pa
        ...
        // Formatar dados no formato "temp_aht;pressao;umidade;altitude"
        sprintf(buffer_lora, "%.1f;%.1f;%.1f;%.1f",
                data.temperature, pressure / 1000.0, data.humidity, altitude);
        printf("Mensagem enviada: %s\n", buffer_lora);

        // Enviar via LoRa
        SendLora((unsigned char*)buffer_lora, strlen(buffer_lora));
...
// Funções LoRa (mantidas iguais, pois já estão corretas)
static inline void cs_select() {
    asm volatile("nop \n nop \n nop");
    gpio_put(PIN_CS, 0);
    asm volatile("nop \n nop \n nop");
